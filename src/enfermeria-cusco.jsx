    ...
    <nav style={{...S.nav,...S.navS}}>
      <div style={S.navIn}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4}} className="dsk-nav">
          ...
          {user
            ? <a href="/panel" style={{marginLeft:8,padding:"10px 18px",borderRadius:14,fontSize:13,fontWeight:800,color:C.brandDark,background:"rgba(63,184,201,.08)",border:`1px solid rgba(63,184,201,.25)`,display:"inline-flex",alignItems:"center",gap:8,textDecoration:"none"}}><Icon name="nurse" size={15} color={C.brandDark}/>Panel</a>
            : <button onClick={()=>setShowLogin(true)} style={{marginLeft:8,padding:"10px 18px",borderRadius:14,fontSize:13,fontWeight:800,color:C.muted,background:"rgba(96,112,137,.07)",border:`1px solid ${C.line}`,display:"inline-flex",alignItems:"center",gap:8,cursor:"pointer",fontFamily:"inherit"}}><Icon name="nurse" size={15} color={C.muted}/>Personal</button>
          }
          <button onClick={onReq} style={{marginLeft:8,padding:"14px 28px",borderRadius:18,fontSize:15,fontWeight:800,color:"#fff",background:"linear-gradient(135deg,#35b5c5,#227a74)",boxShadow:"0 16px 34px rgba(47,156,149,.30)",border:"1px solid rgba(255,255,255,.22)",display:"inline-flex",alignItems:"center",gap:10,letterSpacing:".01em",cursor:"pointer",fontFamily:"inherit"}}>Agendar Cita <Icon name="arrow" size={17} color="#fff"/></button>
        </div>
      ...
      {open&&<div style={{background:"rgba(255,255,255,.98)",borderTop:"1px solid #d6e6e5",padding:16,animation:"slideIn .25s ease-out"}} className="mob-menu">
        ...
        {user
          ? <a href="/panel" style={{display:"flex",alignItems:"center",gap:8,padding:"12px 16px",borderRadius:12,fontSize:14,fontWeight:700,color:C.brandDark,cursor:"pointer",textDecoration:"none",marginTop:4}}><Icon name="nurse" size={16} color={C.brandDark}/>Ir al panel</a>
          : <button onClick={()=>{setOpen(false);setShowLogin(true);}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"12px 16px",borderRadius:12,fontSize:14,fontWeight:600,color:C.muted,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",marginTop:4}}><Icon name="nurse" size={16} color={C.muted}/>Acceso personal</button>
        }
        ...
        <button onClick={()=>{setOpen(false);onReq()}} style={{width:"100%",padding:"12px 18px",borderRadius:14,background:"linear-gradient(135deg,#35b5c5,#227a74)",color:"#fff",fontWeight:800,fontSize:14,marginTop:8,border:"1px solid rgba(255,255,255,.18)",boxShadow:"0 12px 26px rgba(47,156,149,.24)",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:8,cursor:"pointer",fontFamily:"inherit"}}>Agendar Cita <Icon name="arrow" size={17} color="#fff"/></button>
      </div>}
